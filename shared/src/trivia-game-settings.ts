...
export class TriviaGameSettings {
    ...
    getRating(): TriviaGameRating {
        ...
        if (this.roundSettings.length != DEFAULT_GAME_SETTINGS.roundSettings.length) {
        ...
        for (let roundIndex = 0; roundIndex < this.roundSettings.length; roundIndex++) {
            ...
            const defaultRoundSettings = DEFAULT_GAME_SETTINGS.roundSettings[roundIndex];
...
// default game settings (also used as the benchmark for game rating)
export const DEFAULT_SINGLE_ROUND_SETTINGS = new TriviaRoundSettings(TriviaRoundType.Standard, [], 6, 5, 200, { [TriviaClueBonus.Wager]: 1 });
...
export const TEST_SINGLE_ROUND_SETTINGS = new TriviaRoundSettings(TriviaRoundType.Standard, [], 3, 2, 200,
    { [TriviaClueBonus.Wager]: 1, [TriviaClueBonus.AllPlay]: 1 });
export const TEST_DOUBLE_ROUND_SETTINGS = new TriviaRoundSettings(TriviaRoundType.Standard, [], 3, 2, 400,
    { [TriviaClueBonus.Wager]: 1, [TriviaClueBonus.AllPlay]: 1 });
export const TEST_FINAL_ROUND_SETTINGS = new TriviaRoundSettings(TriviaRoundType.Standard, [], 1, 1, 0,
    { [TriviaClueBonus.AllWager]: 1 });
export const TEST_ROUND_SETTINGS = [TEST_FINAL_ROUND_SETTINGS];
export const TEST_GAME_SETTINGS = new TriviaGameSettings(2000, TriviaGameDifficulty.Easy, 7, 7, 7, TEST_ROUND_SETTINGS);