...
## _Version 1.1, co-developed by @snzow_

### Improved game timing

- Massively reduced downtime between game events, especially online with many players: game timers no longer wait on every player's connection, and the host's voice timing is now accurate (fixing long silences after voice lines)
- Fairer play on slow connections: buzzers and response windows now unlock at the same instant for every player and always grant their full duration, with last-second buzzes and submissions no longer lost due to latency
